...
const GraphNetwork = ({ isDarkMode }) => {
  ...
  const linksWithPositions = useMemo(
    () => links.map((l) => ({
      ...l,
      sourcePos: nodePositions.find((n) => n.id === l.source),
      targetPos: nodePositions.find((n) => n.id === l.target)
    })),
    [links, nodePositions]
  );
    ...
    /* Graph Area - Takes remaining space */
      ...
      <div style={{ flex: 1, position: "relative", padding: "8px", minHeight: 0 }}>
        <svg width="100%" height="100%" viewBox="0 0 400 300" preserveAspectRatio="xMidYMid meet">
          {linksWithPositions.map((link, idx) => <line
    key={idx}
    x1={link.sourcePos?.x}
    y1={link.sourcePos?.y}
    x2={link.targetPos?.x}
  ...
  ].map((item) => <div key={item.label} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
