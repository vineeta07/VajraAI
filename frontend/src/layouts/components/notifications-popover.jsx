...
export function NotificationsPopover({ data = [], sx, ...other }) {
  ...
  >
          ...
          {totalUnRead > 0 && <Tooltip title=" Mark all as read">
        ...
        <Scrollbar fillContent sx={{ minHeight: 240, maxHeight: { xs: 360, sm: "none" } }}>
          <List
    disablePadding
    subheader={<ListSubheader disableSticky sx={{ py: 1, px: 2.5, typography: "overline" }}>
                New
              </ListSubheader>}
  >
            {notifications.slice(0, 2).map((notification) => <NotificationItem key={notification.id} notification={notification} />)}
          </List>
  ...
  >
            {notifications.slice(2, 5).map((notification) => <NotificationItem key={notification.id} notification={notification} />)}
          </List>
        </Scrollbar>
...
function renderContent(notification) {
  const title = <Typography variant="subtitle2">
      {notification.title}
