...
export function AnalyticsOrderTimeline({ title, subheader, list, sx, ...other }) {
  ...
  >
        {list.map((item, index) => <Item key={item.id} item={item} lastItem={index === list.length - 1} />)}
      </Timeline>
    </Card>;
}
function Item({ item, lastItem, ...other }) {
  return <TimelineItem {...other}>
      <TimelineSeparator>
        <TimelineDot
    color={item.type === "order1" && "primary" || item.type === "order2" && "success" || item.type === "order3" && "info" || item.type === "order4" && "warning" || "error"}
  />
